...
mea_parser.add_argument('--lolv2_real', action='store_true', help='measure lol_v2_real dataset')
mea_parser.add_argument('--lolv2_syn', action='store_true', help='measure lol_v2_syn dataset')
mea_parser.add_argument('--sice', action='store_true', help='measure sice dataset')
...
def metrics(im_dir, label_dir, use_GT_mean):
    ...
    for item in tqdm(sorted(glob.glob(im_dir))):
        ...
        if os_name.lower() == 'windows':
            ...
            name = item.split('/')[-1]
        img_gt = label_dir + name
        if not os.path.exists(img_gt):
            img_gt = img_gt.split('.')[0]+'.PNG'
        im2 = Image.open(img_gt).convert('RGB')
        (h, w) = im2.size
        im1 = im1.resize((h, w))  
        im1 = np.array(im1) 
        im2 = np.array(im2)
        ...
        if use_GT_mean:
            ...
            out = im1.astype(np.uint8)
            save_path = './output/LOLv1gt/' + name
...
if __name__ == '__main__':
    ...
    if mea.lol:
        ...
        label_dir = './datasets/LOLv1/eval15/high/'
    if mea.lolv2_real:
